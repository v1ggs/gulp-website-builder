...
const minifier = function (cb) {
   ...
   // minify sequence
   let min = _fn
      .src(files.output + '/**/*.js', { allowEmpty: true })
      .pipe(_fn.plumber({ errorHandler: _fn.errHandler }))
      // add suffix
...
const main = function (cb) {
   ...
   for (let i = 0, bnds = pkgObj.length; i < bnds; i++) {
      ...
      for (let j = 0, cfgNo = _type.length || 1; j < cfgNo; j++) {
         ...
         // file output
         let file = _fn
            .merge2(_fn.src(pkgObj[i].src, { allowEmpty: true }))
            .pipe(_fn.plumber({ errorHandler: _fn.errHandler }))
            .pipe(_fn.gulpif(checkSourcemaps, _fn.smaps.init()))
            // concatenate files
            .pipe(_fn.concat(filename))
            // transpile
            .pipe(_fn.gulpif(transpile, babel(babelCfg[babelCfgIndex])))
            // if minifying, don't write sourcemaps, they break some plugins
            .pipe(
               _fn.gulpif(
                  !cfg.build.minify,
                  _fn.gulpif(checkSourcemaps, _fn.smaps.write('./sourcemaps'))
               )
            )
            // output
            ...
            .pipe(
               _fn.gulpif(
                  ...
                  _fn.gulpif(!checkSourcemaps, stripDebug())
               )
            )
            // minify
            .pipe(_fn.gulpif(cfg.build.minify, uglify(minCfg)))
            // write sourcemaps
            .pipe(
               _fn.gulpif(
                  cfg.build.minify,
