...
const postCssPlugins = function () {
    ...
    if (cfg.build.purge) { plugins.push(postcssPurgecss(purgeCssOptions())); }

    if (cfg.build.fixFlexbugs) { plugins.push(fixFlexbugs(cfg.fixFlexbugs)); }

    if (cfg.build.prefix) { plugins.push(autoprefixer(cfg.autoprefixer)); }

    if (cfg.build.combineMQ) { plugins.push(sortMediaQueries({ sort: cfg.build.combineMQ })); }
...
const buildSource = function () {
    ...
    return _fn.src(files.src, {
        ...
        .pipe(sass({
            ...
            indentWidth: 3, // max 10, default: 2
        }))
        .pipe(_fn.ren(function (path) {
...
const minifier = function (cb) {
    let header = _fn.headerCheck();
    let minify = _fn.src(files.output + '/**/*.css', {
        allowEmpty: true,
        base: files.output + '/'
    })
        .pipe(_fn.plumber({ errorHandler: _fn.errHandler }))
